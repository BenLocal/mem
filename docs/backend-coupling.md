...
### 6.6 Phase 5 — in progress (2026-05-17 →)

Two parallel tracks: **(a)** the 4 non-trait pain fixes from Phase 4 §6.5 (pain #1/#3/#4/#5), and **(b)** Backend umbrella trait + service migration so callers depend on `Arc<dyn Backend>` instead of `Arc<Store>`.
...
Doc-original tail items (env factory, public-API hiding, Schema IR, queue module spinout) park behind the umbrella + pain fixes — they're refactors on top of the trait, not blockers for it.
