...
### 5.1 LT-1: 整体 trait 抽象 + 全调用点替换 — ✅ shipped (Phase 5, 2026-05-18)
...
**实际成果**: `pub trait Backend` 在 `src/storage/backend.rs` 落地，supertraits 集齐 §3.2 列的 9 个 sub-trait + Send/Sync/'static，blanket impl `<T> Backend for T where T: 9-traits`。3 个 service (`capability_capsule_service` / `entity_service` / `transcript_service`) + 5 个 worker (`embedding` / `transcript_embedding` / `vacuum` / `decay` / `auto_promote`) 全部改吃 `Arc<dyn Backend>`，`app.rs` 在 setup 里 upcast 一次 (`let store: Arc<dyn Backend> = store_concrete;`)。pipeline 里 QW-5 时建的 narrow `GraphRead` / `SessionStore` traits 删除（trait upcasting 1.86+ 让 `&dyn Backend → &dyn GraphStore → &dyn SessionStore` 直接走），pipeline 现在直接吃 storage 层 sub-traits。CapsuleStore trait 加了 4 个原来在 Store 上的 read 方法 (`list_wings` / `capsule_stats` / `get_taxonomy` / `list_capability_capsules_in_scope`) 让 service 能从纯 trait 走全路径；InMemory + Postgres 两个 backend 都补齐实现。全测试绿（182 lib + 125 integ）。
...
#### Pain #1: ~~`version: u64` ↔ Postgres BIGINT (signed i64)~~ ✅ resolved (Phase 5, 2026-05-17)
...
**Fixed**: domain `version` 改 `i64`，Lance schema `DataType::UInt64 → Int64`，DuckDB read `row.get::<_, i64>(6)`，Postgres bind 直接 `.bind(memory.version)`（无 try_from）。全 backend parity 26/26 测试绿。
...
> **第二个后端实证（ClickHouse spike，2026-06-25）**：同样的 11-子-trait + blanket-impl 插拔机制又接了一个 OLAP 后端(`ClickHouseBackend`，P1–P6，feature-gated)，对真实 ClickHouse 26.5 跑通 e2e（10/10 gated 测试）。它的 pain inventory（update-heavy↔append 的 ReplacingMergeTree 版本化重插、无事务的 non-atomic claim、无 BM25 的软 FTS parity、`''`-as-absent、connect 解析 URL creds 等 25 项）独立维护在 [`clickhouse-backend.md`](./clickhouse-backend.md) §10，与本文 §6.5 的 Postgres 5 痛点呼应——再次印证伞 trait 不需要为新后端 reshape。
...
### 6.6 Phase 5 — ✅ shipped (2026-05-17 → 2026-05-18)

Two parallel tracks both landed: **(a)** the 4 non-trait pain fixes from Phase 4 §6.5 (pain #1/#3/#4/#5), and **(b)** Backend umbrella trait + service migration so callers depend on `Arc<dyn Backend>` instead of `Arc<Store>`.
...
| Pain #4 — atomicity contract on trait doc | spec non-atomic on `apply_feedback` + `replace_pending_with_successor`, callers may observe partial state on Lance crash | ✅ `cec339d` |
| LT-1 umbrella `Backend` trait | aggregate 9 sub-traits behind `pub trait Backend` + blanket impl; CapsuleStore extended with `list_wings` / `capsule_stats` / `get_taxonomy` / `list_capability_capsules_in_scope` (the 4 methods services needed but were Store-only) | ✅ (this commit) |
| LT-1 service migration | `capability_capsule_service` / `entity_service` / `transcript_service` + 5 workers all hold `Arc<dyn Backend>`; pipeline narrow `GraphRead` + `SessionStore` deleted (storage sub-traits now used directly via trait upcasting); `app.rs` upcasts at construction | ✅ (this commit) |

**Validation**: full test suite green — 182 lib unit tests + 125 integration tests across 17 suites including capsule_store_parity 28/28 (Lance + InMemory). `cargo fmt --check` + `cargo clippy --all-targets -- -D warnings` clean on both default and `--features postgres` builds.

**What's left for Phase 5+ tail items** (parked, no longer blocking):

- ~~`DuckDbQuery` / `LanceStore` → `pub(crate)`~~ ✅ shipped (2026-05-18). Module hiding required a dead-code cleanup pass first: 22 LanceStore READ methods (across `capability_capsules.rs`, `entities.rs`, `graph.rs`, `transcripts.rs`) became orphaned when `DuckDbQuery` took over reads — every `lance.xxx` read was unreachable; only `query.xxx` was called. Deleted them along with their lance-side round-trip tests (whose write-then-read invariant is canonically tested in `duckdb_query/*/tests` instead). After cleanup, flipping `pub mod lance_store` / `pub mod duckdb_query` → `pub(crate)` blocks all cross-crate access to the concrete halves. External code goes through `Backend` or a sub-trait, period.
