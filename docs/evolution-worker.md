...
- (b) **新 DuckDB native 表 `evolution_candidates`**（类比 `embedding_jobs` 同为 native 非 Lance 表）：一行一个候选操作，JSON 存参与方与参数，列存 `evidence`、`consecutive_cycles`、`status(pending|ready|executed|cancelled)`——单行原子更新，审计直观。
...
| 信号 | 来源（已核实） | 喂给算子 |
|---|---|---|
| 召回频率 / 新近度 | `last_used_at` / `last_recalled_at` 列（`src/domain/capability_capsule.rs:242` 起），`bump_last_used_at`（`src/storage/duckdb_query/decay.rs:120`），`last_used_worker` 批次 | ③④⑤⑥ |
...
| 图读 / 点时查询 | `neighbors_within(node, max_hops, as_of)`（`MAX_HOPS_CAP=3`）、`graph_stats` | `src/storage/duckdb_query/graph.rs` | 回滚溯源、审计 |
...
| 衰减 | `start_decay_worker` / `apply_time_decay` / `DECAY_RATE_PER_DAY` / `decay_delta` | `src/worker/decay_worker.rs:11/:20/:8`、`src/storage/duckdb_query/decay.rs:32`、`src/domain/capability_capsule.rs:123` | ⑤ 的作用对象；垂死过滤 |
| 检索强化时间戳 | `last_used_at` / **`last_recalled_at`**（idle 判定专用）/ `bump_last_used_at` / `last_used_worker` | `src/domain/capability_capsule.rs:237–248`、`src/storage/duckdb_query/decay.rs:120`、`src/worker/last_used_worker.rs` | 召回频率信号；⑥ 共召回批次来源 |
...
### 8.2 新表 `evolution_candidates`（DuckDB native，类比 `embedding_jobs`）
