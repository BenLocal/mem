...
- **触发与动作**（全部有界、低频、走可审计通道）：
  - 簇稳定 + 高召回（簇内 `last_recalled_at` 活跃占比 > 0.5；**不用 `last_used_at`**——那是 decay 时钟，每小时 sweep 会把全部活跃行刷成 now，2026-07-06 审计 #2 修正）→ 簇成员 `confidence` 微升（+0.02/周期，上限 0.9）——**通过既有 `feedback_events` 通道写**（新增系统侧 `kind`，或复用 `applies_here` 加 `note="evolution:cluster_stability"`，取舍见 §12），保证与人工反馈同一审计流；
...
- **缺口 1 —— 胶囊↔胶囊共召回成边**：K9 只增强已存在的边，不创造新边。本算子消费同一事件流（或直接读 `last_used_worker` 的批次——同批 `bump_last_used_at` 的 id 集合即一次共召回，实现取 `last_recalled_at` 批次戳（decay-sweep-proof，2026-07-06 审计 #2 修正）），对共召回 ≥`min_corecall` 次（跨 ≥K 期）的胶囊对写 `co_recalled_with` 边（`extractor="evolution"`，先例：cooccurrence 的 `extractor="cooccurrence"`）。新边进入 O4 graph-boost 的 1-hop 扩展，直接抬升检索连带召回。
