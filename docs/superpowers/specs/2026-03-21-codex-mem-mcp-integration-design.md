...
| `memory_review_accept` | `POST /reviews/pending/accept` | 审核通过（无编辑）。 |
| `memory_review_reject` | `POST /reviews/pending/reject` | 审核拒绝。 |
| `memory_review_edit_accept` | `POST /reviews/pending/edit_accept` | 修改 pending 内容后接受并生成 active 后继。 |
