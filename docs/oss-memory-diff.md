...
| 项 | 主题（来源） | 研判 | 论据与触点 |
|---|---|---|---|
| **I1** | **PPR 图传播评分替代 flat graph boost**（HippoRAG 2, arxiv 2502.14802） | 🟢 P1 立项 ★本轮最高价值 | 有 mem 自己的实证支撑：G2 落地时 LoCoMo n=50 实测 **multi-hop 恒 0.462、任何尺度的 flat 邻居 boost 都反伤 top-5**（§8 G2）。HippoRAG 2 的解法正是把「邻居一律 +12」换成 Personalized PageRank：query 命中的锚胶囊做 seed（权重 = 各自 RRF 分），沿 `graph_edges` 有界 power iteration（~10 轮，reset→seed），节点按收到的 PPR 质量给 boost——多跳节点拿到的是**衰减过的传播分**而不是 0/12 阶跃。零 LLM、纯 Rust、图和边已经全在（H1 连边 + 实体边 + K1 事实边），改动面 = `retrieve.rs::compute_graph_boosts` 一处 + 复用 `neighbors_within` 的邻接读。评测现成：`LOCOMO_GRAPH=1` 姿势直接出 before/after 对照，验收线 = multi-hop any@5 回升且 single-hop 不掉。 |
