...
| **G2** | 图驱动检索（多跳候选生成,HippoRAG/Graphiti） | ✅ 落地（2026-07-03） | `rank_with_hybrid_and_graph` 新增第 6 参 `capsules: Option<&dyn CapsuleStore>`——expand_graph 时把「有图 boost 但不在 pool ∪ hybrid」的胶囊按 id 水合并入候选（Active+未过期，失败软降级），池被 `MEM_RECALL_POOL_LIMIT` 限时图真正成为第三召回通道（`tests/graph_channel.rs` 两测锁行为+姿势）。**边权两级化**：带 cosine confidence 的相似铸造边（H1 related_to/O2 suspected_supersede）用 tiebreaker 级 `CONFIDENCE_EDGE_BOOST=4×conf`，实体/血缘边保持 `GRAPH_BOOST=12`（`incident_edges_for_nodes` 返回加 confidence 三元组,三后端）。**LoCoMo n=50 实测 trade-off**（boost 12 档）：open-domain any@5 +8pt、any@10 +4pt,但 multi-hop any@5 **-15pt**（RRF 相邻名次差 <3 分,任何尺度的邻居 boost 都会挤 top-5——三种 boost 尺度下 multi-hop 恒 0.462）→ 头条基准姿势不开图（`LOCOMO_GRAPH=1` 可复现图姿势）,生产按请求 `expand_graph` 自选。 |
