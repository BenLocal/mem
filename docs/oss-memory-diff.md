...
O6a/b/c 衡量召回质量都是**离线**（CI / 一次性 bench）。线上跑着的服务**在做什么**——O5 脱敏到底有没有触发、O7(a) 近重复闸是过敏还是过松、feedback 回流率多少——之前只能 grep 日志。补一个**进程内原子计数器**注册表（`src/metrics.rs`，`once_cell::Lazy` 单例 + `AtomicU64`，零新依赖、不穿 `AppState`），经 `GET /metrics` 返回 JSON 快照（与 `/…/stats` 同风格、只读免鉴权同 `/health`）。计数器名**按管线显式前缀**（mem 有 capsule / transcript 两条并行管线 + episode，裸 `ingest_total` 会误导成全量）:`capsule_ingest_total`/`capsule_search_total`/`transcript_ingest_total`/`transcript_search_total`/`episode_ingest_total`（速率分母）、`redaction_hits`（O5,**单一跨面**计数:capsule compress + 两个 embedding worker + transcript search 任一 `redact_secrets` 返回 `Cow::Owned` 即 +1,不污染纯函数 `redact_all`）、`neardup_flags`（O2/O7(a) 翻 `PendingConfirmation` 成功后 +1）、`feedback_*`（按 6 种 `FeedbackKind` 分桶,typed 路由——新增 variant 是编译错而非静默漏）。**进程本地、重启清零、不持久化**（同 `MEM_MAX_INGEST_PER_SESSION` 语义）,`Relaxed` 序、读路径无锁。埋点全在 choke point（`redact_secrets` / `embedding_worker::flag_if_near_duplicate` / capsule `service::{ingest,ingest_batch,search,submit_feedback,ingest_episode}` / transcript `service::{ingest,ingest_batch,search}`）,坏请求 422 在进 handler 前被拒、不计数（端到端 smoke 验证过）。**触点**：新 `src/metrics.rs` + `src/http/metrics.rs` + capsule/transcript 两条管线各埋点。**风险**:极低（只读端点 + 计数器,不影响任何业务逻辑）。
...
| **G2** | 图驱动检索（多跳候选生成,HippoRAG/Graphiti） | ✅ 落地（2026-07-03） | `rank_with_hybrid_and_graph` 新增第 6 参 `capsules: Option<&dyn CapsuleStore>`——expand_graph 时把「有图 boost 但不在 pool ∪ hybrid」的胶囊按 id 水合并入候选（水合完整对齐 pool 姿势：Active+未过期+**非 Diary**+**非被 Active 后继取代**，tenant 省略时同 service 默认 `local`——2026-07-06 审计 #3/#4/#5 修正；失败软降级），池被 `MEM_RECALL_POOL_LIMIT` 限时图真正成为第三召回通道（`tests/graph_channel.rs` 两测锁行为+姿势）。**边权两级化**：相似铸造边（extractor=`ingest_link`/`o7_neardup_cluster`）用 tiebreaker 级 `CONFIDENCE_EDGE_BOOST=4×conf`，其余边（实体/血缘/人工 K1 事实，**无论是否声明 confidence**）保持 `GRAPH_BOOST=12`——判别键是 extractor 而非 confidence 有无（2026-07-06 审计 ⑥ 修正：旧规则下人工边声明高置信反被降 3 倍）；`incident_edges_for_nodes` 返回 `IncidentEdge{from,to,confidence,extractor}` 结构体（三后端，CH 不再 pair-dedup、0.0 哨兵读回 None——审计 ⑨⑩）。**LoCoMo n=50 实测 trade-off**（boost 12 档）：open-domain any@5 +8pt、any@10 +4pt,但 multi-hop any@5 **-15pt**（RRF 相邻名次差 <3 分,任何尺度的邻居 boost 都会挤 top-5——三种 boost 尺度下 multi-hop 恒 0.462）→ 头条基准姿势不开图（`LOCOMO_GRAPH=1` 可复现图姿势）,生产按请求 `expand_graph` 自选。 |
...
| 项 | 主题（来源） | 研判 | 论据与触点 |
|---|---|---|---|
| **H1** | **ingest 时近邻连边**（A-Mem 链接网络思想的零-LLM 切片） | ✅ 落地（2026-07-02） | A-Mem 每条新记忆 LLM 建链 + 触发旧记忆演化；mem 的零-LLM 等价切片：embedding worker 嵌完新胶囊后（`flag_if_near_duplicate` 同一时机）对 cosine ∈ [连接阈值, 近重复阈值) 的 top-k 邻居写 `related_to` 边（extractor=`ingest_link`，非 review、纯连通性）。直接喂 O4 graph boost，也为 G2「图第三召回通道」预铺边密度。风险低：不动状态、不动内容，只加边。 |
| **H2** | **反馈笔记 → 修订提案**（MemOS「自然语言纠错」的 review-gated 版） | ✅ 落地（2026-07-02） | MemOS 支持用 NL 反馈直接修正记忆；mem 的对应件已经齐了 90%：`outdated`/`incorrect` 反馈的 `note` 本就 verbatim 落 `feedback_events`，E5 ③ refine 已把 `outdated≥2` 当冲突信号——差的只是把这些 note **抄进 refine 占位的冲突证据清单**（现在只写计数），reviewer 一眼看到"为什么旧"。零 LLM、零新表，改动面 = refine 检测收集 note + `SynthesisTask::Refine` 原料。 |
| **H3** | **LoCoMo bench harness**（mem0/MemOS/Zep 通用第二基准） | ✅ 落地 + 真集数已产出（2026-07-02） | 赛道自报口径已从单一 LongMemEval 变成 LongMemEval+LoCoMo（+PersonaMem/PrefEval）双基准起步；mem 只有 LongMemEval parity（O6c，N=50 出数中）。复用 `mempalace_bench` 骨架（fresh Store→ingest→hybrid→recall@k），换数据装载器即可。对外可比性直接翻倍。 |
| **H4** | **Skill 结晶**（MemOS crystallized skills / LangMem prompt optimization） | ✅ **检测端** 2026-07-09 + **结晶端** 2026-08-01（`mem crystallize`） | **⚠️ 2026-08-01 订正**：本行原标「✅ 落地（2026-07-09）」是**高估**——那批只落了检测端（姊妹簇判定 + `workflow_generalize` 占位），而占位胶囊的 content 是一段**给人看的评审指令**（"Review task: … write ONE reusable step-by-step workflow"），真正的结晶要人手写；`ReviewSynthesisBackend` 按设计 "performs NO generation"。结晶端 2026-08-01 由 `src/cli/crystallize.rs` 补齐：一次性 CLI（常驻 worker 保持 LLM-free，§6.2 的 `local`/`api` 后端仍在 `config.rs` 解析期被拒）、O7(c) 三重 fail-safe、默认 dry-run `--accept` 才写、纯 HTTP 客户端零新端点、簇长大按 `|A∩B|/min ≥ 0.5` supersede。**顺带修一个线上缺陷**：占位符（Workflow 型 PendingConfirmation）此前正被 `compress.rs` 当作 `suggested_workflow` 发给 agent（`retrieve.rs` 只扣 4 分不排除），即评审工单被当成可执行流程——`is_authoritative_workflow` 现要求该槽位必须 `Active`。验收 `tests/crystallize.rs`。以下为检测端原始记录：**落地形态偏离原设想（以代码为权威）**：判定器不挂在 `detect_generalize`（0.80 粗簇取主题交集会被杂成员稀释），而是挂在 `detect_merge` 的 0.88 细簇上——`is_procedural_sibling_cluster`（零 LLM 结构信号：`fact_anchors` = commit-sha 形 token［剥 mem_ id、排纯十进制］∪ code_refs，成员两两锚点近不相交且各有锚点）。命中 = 「同一流程的 N 次执行」：①**merge 改道**（该簇绝不合并——2026-07-09 实测 reranker 相关性/去重两种框架都区分不了姊妹与重复，0.94-0.99 无分离带；改道同时饿死在飞的旧 merge 候选，因为 pending 候选只在有活提案匹配时才执行）；②成员数 ≥ generalize_min_n 时改提 `workflow_generalize` 占位（Workflow 型 PendingConfirmation + `evolution:workflow` tag + generalizes 血缘，评审任务=写一条可复用步骤化流程），§11 rollback 走占位符共享臂。无锚点簇 fail-open 走旧 merge 路径。验收 `tests/evolution_workflow_generalize.rs`。 |
...
**H1–H3 已落地（2026-07-02，以代码为权威）**：H1 = `embedding_worker::link_related_neighbors`——嵌完 Active 胶囊后对 cosine ∈ [`MEM_INGEST_LINK_THRESHOLD`(默认 0.80), 上界) 的 top-4 邻居写 `related_to` 边——上界 = neardup 阈值（O2 lane 开时，该 lane 拥有顶带）或 ∞（lane 关时，2026-07-06 审计 ⑬：否则 cos≥0.92 的最强邻居落进无人认领的死区）；邻居向量按当前 model 过滤（审计 ⑭，换模型迁移中的旧向量余弦无意义）（extractor=`ingest_link`，cosine 入 edge confidence），opt-in `MEM_INGEST_LINK_ENABLED`（默认关，O2 同姿势），跑在 O2 检查之后、best-effort；H2 = `execute_refine` 经新 `CapsuleStore::list_feedback_for_memory`（lance/pg/ch/in-memory 四实现）把 `outdated` 反馈的 verbatim note 追加进占位冲突证据（`outdated note: …` 行），读失败退化为仅计数；H3 = `tests/locomo_bench.rs`（`#[ignore]`）镜像 mempalace_bench 口径——session 级 evidence recall@k、category 1-4（5 对抗题排除，同 `_abs`）、`LOCOMO_SAMPLE` 分层采样（默认 50）、per-conversation 共享 store（LoCoMo QA 共享 haystack，异于 LME 的 per-question）、真集 `locomo10.json`（2.8MB gitignore drop-in，10 会话 1986 QA）缺失时回退 committed synthetic subset；合成集 e2e 10s 验证通过；**真集公开数（2026-07-02，n=50 跨全部 10 会话 category 分层，36min）：any@5=0.660 / recall@5=0.607 / recall@10=0.688 / mrr=0.502**——single-hop 0.917 / temporal 0.846 强，**multi-hop 0.462 / open-domain 0.417 弱**（多跳弱 = G2「图第三召回通道」的实证动机）。注意首版采样有过偏差：category 桶内文件序导致 50 题全落前 2 会话，修为「occurrence-within-conversation 优先」的跨会话轮转后才出数（README 引用的即修正后口径）。两基准数已进 README『Published retrieval numbers』。
...
## 10. I-series 跟进（2026-07-08，第四轮赛道扫描：Hindsight / memU / Memori / Honcho / MemOS local-plugin 2.0 / OmniMemEval）

> 扫描动机：H-series（07-02）之后一周内赛道两个新主角——Vectorize **Hindsight**（arxiv 2512.12818，三通路记忆 + retain/recall/reflect，LongMemEval 91.4% 自报，PG 后端）与 **OmniMemEval**（MemOS 07-02 发布的 10 数据集 × 14 产品统一评测）；另补扫 H 轮没碰的 **memU**（markdown 文件底座 + 自进化 skill）、**Memori**（MemoriLabs，agent-native / BYODB / LLM 调用拦截捕获）、**Honcho**（plastic-labs，observer/observed 心智建模 + dialectic 推理 API）。
>
> **先说结论——mem 已覆盖对方主打卖点的部分**（别重复造）：Hindsight retain 的 LLM 抽取 ≈ O7 三 lane（mem 是 review-gated 版）、其 4 通道 recall+RRF ≈ mem hybrid+G2（差的两块见 I2/I3）、mental models ≈ E2 generalize；Memori 的 BYODB ≈ mem 三后端、结构化省 token ≈ `MEM_RECALL_STYLE=index`；memU 六记忆类型 ≈ 六 capsule type、增量 sync 幂等 ≈ mine idempotency；Honcho 的异步推导 worker ≈ mem worker 群、representation snapshot ≈ G5 profile；MemOS local-plugin 2.0 的 L1 traces/L2 policies/L3 world model/crystallized skills ≈ transcript 档案/Preference/G5 profile/Workflow。
>
> 本轮真正的增量集中在**召回精度工程**（I1/I2/I3——赛道头部在 recall 侧的三件套：图传播、精排、时间通道），而非记忆形态。

| 项 | 主题（来源） | 研判 | 论据与触点 |
|---|---|---|---|
| **I1** | **PPR 图传播评分替代 flat graph boost**（HippoRAG 2, arxiv 2502.14802） | ✅ 落地（2026-07-08），**LoCoMo 验收结论如实记负** | 落地形态（以代码为权威）：`retrieve.rs::fetch_ppr_frontier`（2-hop 有界 frontier，每 hop 一次批量 `incident_edges_for_nodes`，跨 hop 去重 + 2048 节点熔断）+ `ppr_boosts_from_edges`（无向类权重转移：事实边 1.0、相似边 ⅓×cosine；seed=锚集均匀 reset；α=0.5 有界幂迭代；**逐节点类上限映射** `round(ceiling·min(1,p/p_seed_max))`，ceiling=该节点最强入边的 `edge_base_boost`——只被相似边触达的节点绝对值封在 tiebreaker 尺度，防单类图里归一化抵消类权重）。K9 dynamics 路径不动。7 个单测锁不变量（2 跳可达/距离衰减/hub 稀释/类权重/跨锚累积/纯相似图 tiebreaker 上限/真 Store 2 跳集成）。**落地偏离**：seed 权重用均匀 reset 而非「各自 RRF 分」（`graph_anchor_nodes` 排序去重后 rank 已丢，均匀更干净）。**LoCoMo A/B 实测（n=50, LOCOMO_GRAPH=1, Qwen3 模板后姿势）诚实记录**：graph-off 0.700 / flat graph-on 0.660（multi-hop 0.462）/ PPR 0.640（multi-hop 0.385）——flat↔PPR 差 = **50 题中 1 题翻转**（n=10 探针两者逐位同分），三种 flat 尺度 + 两种 PPR 映射全部同分 ⇒ **该语料上伤害是"集合形状"而非"量级"**；根因是 bench 图纯 H1 相似边（无实体边）= ANN 回声，boost 回声邻居必然挤掉 multi-hop 所需的多样证据 session。原验收线（multi-hop 回升）在此语料上**结构性不可达**——PPR 的增量能力（capsule→entity→capsule 2 跳，生产图 tagged 2470/mentions_file 1035/applies_to 503 边为主）bench 无法测量，由单测锁定、待生产 `expand_graph` 调用实测。头条基准姿势维持 graph-off；图通道唯一算法为 PPR（不留 flat 双实现）。 |
| **I2** | **本地 reranker 精排段**（Hindsight recall 第 5 段 cross-encoder；MemOS/Mem0 同样标配 rerank） | ✅ spike 完成（2026-07-08）→ **改判离线形态**（交互精排 CPU 上不可行） | **Spike 实测（Qwen3-Reranker-0.6B，本机 96 核 CPU）**：①打分质量极佳——中文相关 0.9999/无关 0.0001、英文相关 0.9983/同话题难负例 0.0106，正是 open-domain 需要的判别力；②**f32（candle qwen3.rs，mmap safetensors）单对 ~650-800ms**（8/16/32/48 线程 = 806/708/646/679ms，带宽瓶颈平台期），top-8 ≈ 5.2s——超交互预算（决策线 1s）；③**q8_0 GGUF（candle quantized_qwen3）反而慢 12×**（7.8-12.2s/对，线程越多越慢）——candle 0.9.2 的 QMatMul 量化核为 decode 优化，prefill 走慢路径，量化 lane 判死；④批量 padded 前向不可行——candle qwen3 公开 forward 无 attention-mask 入口，左 padding 会被因果注意力污染；⑤embed_anything（锁定 commit 11b6449）**自带 qwen3 reranker**（prompt 模板/yes-no logits/softmax P(yes) 全齐）但跑在 **ort（ONNX Runtime）** 上且 feature 未开——开启 = 引入原生 C++ 依赖，cross musl target 高风险，弃。**改判：离线 reranker lane**（worker 侧，700ms/对无压力，纯 candle 零新增依赖，配方已验证）：(a) H1 `related_to` 边复核——cross-encoder 分写回 edge confidence、低分边 invalidate，图通道边质量升级（喂 I1 PPR）；(b) O7(a) supersede 提案双向打分（非对称高分 =「B 包含 A」强信号），评审队列信噪比提升；(c) evolution merge/generalize 候选对打分替代纯 cosine 阈值。opt-in 默认关，权重已缓存（/root/.cache/huggingface/manual/Qwen3-Reranker-0.6B，safetensors 1.19GB + q8 gguf 639MB 经代理断点续传落盘）。 |
| **I3** | **时间显式召回通道**（Hindsight 4 通道里的 temporal channel） | 🟡 P2 | mem 的时间性目前只是 ranking 里的 freshness 加分 + 边上的 `kg_timeline`；「上次那个 bug」「6 月改的那版」这类 dev-memory 高频查询没有结构化时间入口。零-LLM 切片：intent 检测中文/英文时间指涉（昨天/上周/6月/上次/last week…）→ 解析成时间窗 → `created_at`/`last_recalled_at` 范围扫描作为第四候选通道并入（同 G2 姿势：软失败、姿势对齐 pool 排除规则）。注意 LoCoMo temporal 0.846 已是强项，该项主要服务真实中文工作流而非基准分——验收用 golden_recall 加时间指涉金标组。 |
| **I4** | **OmniMemEval 第三基准**（MemOS，2026-07-02，10 数据集 × 14 产品） | ⏸ watch | eval 线（O6/H3）已有 LongMemEval+LoCoMo 双基准；OmniMemEval 刚发布、口径含 LLM-judge 重的端到端段，等它稳定后评估「可 session-recall 化的子集」进 `mempalace_bench` 骨架。先挂 watch 不立项。 |
| **I5** | **skill 结晶**（memU self-evolving skills + MemOS crystallized skills，双重印证） | ✅ 收口 = **H4 全闭合**（检测端 2026-07-09 + 结晶端 2026-08-01） | **⚠️ 2026-08-01 订正**：本行原写「收口 = H4 已落地（2026-07-09）」，而当时 H4 只有检测端——所以这个 ✅ 是建立在另一个高估之上的（见 §9 H4 行的订正）。`mem crystallize` 落地后 I5 才真正收口。以下为原始记录：落地契机不是市场信号而是自家数据：I2 merge 闸预演暴露的 5 成员迁移簇既是 merge 误判样本又是「同一流程 N 次执行」的结晶样本——姊妹簇结构判定一次实现两个收益（merge 改道 + Workflow 占位提案）。详见 §9 H4 行与 CHANGELOG。 |

**不做名单（本轮新增，理由留档防止重复调研）**：
- **Memori 式 LLM 调用拦截自动捕获**——client SDK 形态（swap LLM client），mem 的采集面是 Claude Code hook + `mem mine` 双通道，服务端无从拦截也不该拦截；其余能力（结构化抽取/省 token/BYODB）mem 均有对应物。
- **Honcho observer/observed 心智建模 + dialectic 推理 API**——多 peer SaaS 形态，推理层强依赖 LLM（撞零-LLM 默认）；单用户 dev-memory 没有「谁对谁的认知」场景。快照类需求 G5 profile 已覆盖。
- **memU markdown 文件底座**——「记忆即可读文件」是 UX 差异不是能力差距；mem 的 verbatim 存储 + admin UI + wake-up 覆盖同类需求。若真要浏览态，做一次性 `mem export --markdown`（S，不立项）。
- **Hindsight reflect / mental models 网络**——LLM 合成派生结论，≈ E2 generalize 的无审计闸版本；mem 的 review-gated 占位提案是差异化卖点（同 H7 判词），维持不追。
- **「存决策不存描述」rate-distortion 框架**（arxiv 2605.10870）——理论有趣，但 O7(b) 的高信号 cue（决策/因果/error→fix）+ ingest 质量闸已经是它的启发式实现，无新增动作。

**建议批次**：I1 单独一批（纯 `retrieve.rs`，评测现成，先出数）；I2 先 spike（半天确认 candle/embed_anything 能不能跑 Qwen3-Reranker，能→立项，不能→改评估 ONNX 路线再议）；I3 排 I1 之后（同在 retrieve 候选层，避免同文件并行改）；I5 并入 evolution Phase 2 排期；I4 watch。

**来源**（2026-07-08 检索）：Hindsight github.com/vectorize-io/hindsight + arxiv 2512.12818（三通路记忆、4 通道 recall→RRF→cross-encoder、retain/recall/reflect、PG 后端、LongMemEval 91.4% 自报）；HippoRAG 2 github.com/OSU-NLP-Group/HippoRAG + arxiv 2502.14802（KG + query-to-triple linking + Personalized PageRank，多跳/关联性优势）；memU github.com/NevaMind-AI/memU（INDEX/MEMORY/SKILL 三层 markdown、六记忆类型、manifest 增量 sync、LLM-free 嵌入检索、07-15 大版本重构中）；Memori github.com/memorilabs/memori（entity/process/session 三级捕获、LLM 拦截、BYODB、LoCoMo 81.95% 自报）；Honcho github.com/plastic-labs/honcho（peer 范式、(observer, observed) 键控结论存储、dialectic API 分档推理）；MemOS local-plugin 2.0 + OmniMemEval github.com/MemTensor/MemOS（L1/L2/L3/skills 四层、FTS5+向量、LoCoMo 92.34 / LongMemEval 93.40 自报）；mem0 行业报告 mem0.ai/blog/state-of-ai-agent-memory-2026（LoCoMo 92.5 / LME 94.4、<7k tokens/查询自报）。
