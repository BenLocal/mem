...
| **I2** | **本地 reranker 精排段**（Hindsight recall 第 5 段 cross-encoder；MemOS/Mem0 同样标配 rerank） | 🟡 P1-P2，先做可行性 spike | 赛道头部 recall 管线已收敛为「多通道 → RRF → **cross-encoder rerank** → 预算裁剪」，mem 恰好停在 RRF。候选模型 Qwen3-Reranker-0.6B（与在用 Qwen3-Embedding-0.6B 同家族同量级，CPU 可跑）。**风险在实现面**：Qwen3-Reranker 是 causal LM 按 yes/no logit 打分，embed_anything 未必支持，可能要 candle 手写 1-token forward——先 spike 确认再立项。姿势：opt-in `MEM_RERANK_ENABLED` 默认关（加延迟），只精排 top-N（~32）候选；打分模型非生成式用途，不破「零生成-LLM 默认」哲学。验收 = golden_recall + LoCoMo 双基准不掉且 open-domain（现 0.417 最弱项）回升。 |
