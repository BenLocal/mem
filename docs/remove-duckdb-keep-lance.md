...
- ⚠️ **先验隐性前提**：lancedb Rust `Table` 是否也 pin 版本？写后是否要 `checkout_latest()` 才看到新数据？这决定 `refresh/mark_dirty` 机制能否真删。参照 `tests/lance_snapshot_visibility.rs` 探针思路先实测。
...
**Phase 3 —— 删除 + 瘦身** ~2-3d
- 删 `src/storage/duckdb_query/`（~6000 行）、`Store` 的 `query`/`refresh`/`mark_dirty`/`ensure_fresh`（若 Phase 0 证明 lance 自动见新版本）、`types.rs` 的 duckdb `From`、`MEM_DUCKDB_THREADS`、`main.rs` 为 duckdb 加的资源缓解。
...
1. 🔴 **lance 版本可见性**（Phase-0 阻断项）：lancedb Rust `Table` 若也 pin 版本，写后需 `checkout_latest()`；决定 `refresh/mark_dirty` 能否删。**未验证前不进 Phase 1。**
2. 🔴 **decay 写迁移**（不可逆门槛）：lance Rust update 语义 + commit-retry 重证。
3. 🟠 **Tantivy↔Lance 一致性**：双写 hook 的崩溃/重启一致；启动重建 vs 持久化增量由 §6 实测定。
4. 🟠 **jieba build 吞吐**：中文分词可能把 build 拉高 2-5×，§6 主测变量。
5. 🟡 **RRF compose 退化**：去 fused-SQL 损 14-29% 延迟，本地少并发下绝对值小，但需回归确认。
6. 🟡 **上游修复后简化**：长期跟踪 lance changelog，scanner 修好则拆 Tantivy 收敛回单 FTS。
