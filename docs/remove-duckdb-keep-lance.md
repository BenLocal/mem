...
> Status: **✅ 已实现 (2026-06-24)**。五个阶段全部落地，DuckDB 读引擎已删除（`duckdb` crate + `src/storage/duckdb_query/` + `refresh`/`mark_dirty`/`ensure_fresh` + r2d2 读池 + `MEM_DUCKDB_THREADS` + `MEM_READ_ENGINE` 开关全部移除，删除点 commit `f0c3b59`）。读面已全部 lance-native：原计划 11 个 golden 桶之外又迁了约 19 个额外读方法，读面完整。**Phase 2 不可逆门槛已过**——两个 decay 写（`apply_time_decay` / `bump_last_used_at`）已改走 lancedb Rust API `table.update()`（见 `src/storage/lance_store/decay.rs`）。§8 的未决项已收敛（分词器选 jieba、Tantivy 走启动全量重建进 RamDirectory），见各项就地标注。
> 决策方向：**去掉 DuckDB 读引擎，保留 Lance 做写/存，不切 Postgres，不放弃 local-first 单二进制**。本篇是 `duckdb-read-path-strategy.md`（2026-06-15，"保留 DuckDB"的决策）的**反向新决策**——那篇结论是"DuckDB 留着、不值得替"，本篇在掌握"QW-1 已把融合拆成可移植 Rust 原语"+"现网真实量级只有十几万行"两个新事实后，重新判定 Route B 可行并给出执行计划。
> **下文的设计 body 保留为实现记录**；具体落地与设计的小差异以代码为准。
...
### 已解除（route-B 落地后，2026-06-24）
2. ✅ **decay 写迁移**（原不可逆门槛，Phase 2 已过）：`apply_time_decay`/`bump_last_used_at` 已改走 lancedb Rust API `table.update()`，与 ingest 同为单写者，旧的 dual-writer / pruned-stale-base commit race 消失。commit-conflict 由 lance 自带 `UpdateBuilder::execute_with_retry`（~10 次/30s 重快照）处理，外层只留一个薄的 `LanceStore::with_lance_commit_retry`（max 3，重开表重试）兜极端竞争 + 作测试 seam。见 `src/storage/lance_store/decay.rs`。
3. ✅ **jieba docs/sec**（§6 主测变量）：§6 微基准定案——jieba（precision 模式）top-10 召回 overlap 与 cang-jie 持平（0.975）且 build 快 ~1.85×，选 jieba。
4. ✅ **transcript 全量重建耗时**（§6）：实测真实规模（~31k docs）全量重建 <1s、10×（~314k）~6s，远低于 30s 门槛——故走「启动全量重建」兜底，不需持久化增量。
5. ✅ **Tantivy↔Lance 一致性**：选用 `Index::create_in_ram` 进 RamDirectory + 启动全量重建（再由 `MaintenanceStore::rebuild_query_indexes` 触发重建），无磁盘索引、无 stale-index 窗口、无双写一致性问题——source of truth 始终是可重扫的 Lance 表。
