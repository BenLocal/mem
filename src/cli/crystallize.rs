...
pub use super::crystallize_adapters::{GatewaySynthesizer, HttpReviewClient};
...
pub struct CrystallizeArgs {
    ...
    /// Consume deterministic completed-tool-round candidate jobs instead of
    /// legacy evolution placeholders.
    #[arg(long)]
    pub candidate_jobs: bool,

    /// Publish compiled candidate jobs as PendingConfirmation Workflow
    /// proposals. This never activates a Skill; human bundle review is a
    /// separate server-side operation.
    #[arg(long)]
    pub propose: bool,

    /// Maximum candidate jobs to process in one explicit run.
...
pub async fn run(args: CrystallizeArgs) -> i32 {
    ...
    if args.candidate_jobs {
        if args.accept || !(1..=8).contains(&args.limit) {
            eprintln!(
                "candidate-job crystallization forbids legacy --accept and requires --limit 1..=8"
            );
            return 2;
        }
        return super::crystallize_candidates::run(args.remote, cfg, args.propose, args.limit)
            .await;
    }
    if args.propose {
        eprintln!("--propose is valid only with --candidate-jobs");
        return 2;
    }
