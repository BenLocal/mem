...
pub(crate) fn row_to_record(
    ...
    Ok(CapabilityCapsuleRecord {
        ...
        last_recalled_at: row.try_get("last_recalled_at").map_err(sqlx_err)?,
...
pub(crate) const SELECT_COLUMNS: &str =
    ...
    last_used_at, last_recalled_at, expires_at";
...
impl CapsuleStore for PostgresCapsuleStore {
        ...
        for m in memories {
            sqlx::query(
                "INSERT INTO capability_capsules (\
                    ...
                    created_at, updated_at, last_validated_at, last_used_at, last_recalled_at, expires_at) \
    ...
    async fn replace_pending_with_successor(
        ...
        .bind(&successor.last_recalled_at)
    ...
    async fn delete_capability_capsule_hard(
        ...
        // `capability_capsule:<id>` edges in active reads (graph reads scan
        // graph_edges with no join to the parent capsule). A capsule can be the
        // `to_node` of an edge (e.g. a `suspected_supersede` new→canonical edge,
        // or a `contradicts` edge), so a FROM-only close would strand that
        // incoming edge. Done IN the transaction so the whole hard-delete stays
        // atomic. Matches the lance/clickhouse cascade.
