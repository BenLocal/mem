...
- `mcp__mem__capability_capsule_propose_experience` — **misleadingly named**, writes to the `episodes` table (NOT the review queue) for downstream workflow extraction. Prefer the `_ingest` + `write_mode=propose` path above when the goal is human review before promotion
