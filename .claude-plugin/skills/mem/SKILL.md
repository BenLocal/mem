...
(e) **About to persist a learning** — finished any non-trivial task: debugging session, fix, gotcha, design decision, API quirk, configuration choice. **Default to calling `capability_capsule_propose_experience`** — it lands in the review queue (`list_pending_review` shows it; `review_accept` promotes to active pool), so false positives are cheap and discarded by the review gate. Bias toward proposing when in doubt; under-proposing means the next session re-derives context that was already learned. Skip only when (i) the commit is a pure typo / formatting / dependency bump, or (ii) the same fact was already captured by an earlier capsule in this session. Use the stronger `capability_capsule_ingest` only when the **user** explicitly asks to save a verbatim fact (via `/mem:save` or "remember this exactly") — that path writes straight into the active pool, no review gate. The verbatim-rule applies in both: write the full explanation as `content`, not a refined summary.
