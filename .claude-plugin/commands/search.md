...
Run a memory search via the `mcp__mem__memory_search` tool with the user's query as `$ARGUMENTS`.

Procedure:

1. Treat the entire `$ARGUMENTS` as the query string. If empty, ask the user what they want to search for.
2. Call `mcp__mem__memory_search` with `tenant` (default `local` from `MEM_TENANT`), `query`, and `limit: 10`.
...
4. If a returned memory directly answers the user's question, after using it, call `mcp__mem__memory_feedback` with `feedback_kind: "useful"` for that memory_id. If it's just relevant context, use `applies_here` instead. **At most one feedback signal per memory per session.** Don't fire feedback on memories you only skimmed.
