...
- **Storage layer (`src/storage/`)**: DuckDB (bundled, single file) wrapped in `Arc<Mutex<Connection>>` — every DB call serializes through one mutex. This is the de-facto concurrency boundary; treat it as "one transaction at a time." Graph layer is dual: `IndraDbGraphAdapter` (in-memory `MemoryDatastore`) + `LocalGraphAdapter` fallback. ANN sidecar lives in `vector_index.rs` (usearch HNSW, single-file `<MEM_DB_PATH>.usearch` + meta JSON; rebuildable from DuckDB on every startup mismatch).
